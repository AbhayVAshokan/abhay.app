...
const Mobile = ({ document }: PDFViewerProps) => {
  const pageOneRef = useRef<HTMLCanvasElement>(null);
  const pageTwoRef = useRef<HTMLCanvasElement>(null);
  const [{ blob }] = usePDF({ document });
  pdfjs.GlobalWorkerOptions.workerSrc = window.location.origin + "/pdfjs-dist/build/pdf.worker.min.mjs";

  useEffect(() => {
    if (!blob || !pageOneRef.current || !pageTwoRef.current) return;

    const renderPDF = async () => {
      const arrayBuffer = await blob.arrayBuffer();
      const pdf = await pdfjs.getDocument({ data: arrayBuffer }).promise;
      const pages = [pageOneRef.current, pageTwoRef.current] as HTMLCanvasElement[]

      for (let pageNumber = 1; pageNumber <= 2; pageNumber++) {
        const page = await pdf.getPage(pageNumber);
        const viewport = page.getViewport({ scale: 1.5 });

        const canvas = pages[pageNumber - 1];
  ...
  return <div
    className="z-10 auto gap-1 flex flex-col fixed"
...
const PDFViewer = ({ document }: PDFViewerProps) => {
  const isMobileOrTablet = () => {
    if (/Mobi/i.test(window.navigator.userAgent)) { return true; }
  ...
  const Component = isMobileOrTablet() ? Mobile : Desktop;
