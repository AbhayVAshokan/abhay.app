...
export const LucideIconToPDF = ({
  Icon,
  color = "black",
  size = 24,
}: LucideIconToPDFProps) => {
  ...
  const svgRects = Array.from(svgElement.querySelectorAll("rect")).map(
    (rect, index) => (
      <Rect
        key={index}
...
const IconText = ({ style, text, Icon }: IconTextProps) => {
  ...
  return (
    <View style={cn(styles.container, style)}>
      <LucideIconToPDF Icon={Icon} size={10} color={theme.darkGray} />
      <Text style={[styles.text, { color: theme.darkGray }]}>{text}</Text>
    </View>
  );
};
